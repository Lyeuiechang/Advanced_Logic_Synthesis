...
{
    if (argc != 5) {
       printf("Usage: ./map ¡Vk <LUT Size K> <Original testcase file path> <Your output file path>\n");
       return 1;
    }
    K = atoi(argv[2]);
    realpath(argv[3], input_path);
    realpath(argv[4], output_path);
    ifs.open(input_path,ios::in);
...
void stageone()
{
    ...
    for(i=numberofinputs;i<node_size;i++)
    {
        ...
        while(temp_model.node[i].inputs.size() > 2)
        {
            ...
            temp_gate.name = temp_model.node[i].name + patch::to_string(counter);
